...
> ⚠️ **이 문서는 최초 구현 계획서(역사적 기록)다.** 실제 구현은 이후 상당히 발전했다(핵심 용어 흐름형 재분류, 통합 검색, 꾸러미별 OG 카드, 영상 영화관 모드 등). **현재 아키텍처·구조·기능은 [README.md](../README.md)를 우선 참조**할 것. 아래 내용 중 시드 데이터(ab12/cd34), 디렉터리(_prototype 등), 단계 계획은 현재와 다를 수 있다.
...
**In scope**
- 홈 갤러리(필터 + 카드 그리드), 꾸러미 뷰어(목차 + 4타입 본문), 활용 안내 모달(교사용 사용법). 데이터 구조 스키마는 앱이 아니라 docs(이 문서·SHEET_TEMPLATE)에만 둔다.
...
3. `선사 시대와 고조선` 카드 클릭 → 뷰어(`/ab12`) 진입, 첫 항목 자동 선택.
...
```
   교사(브라우저)
   ────────────
   GET kit.dgedu.link/ab12 ─▶ 앱 경로와 동일(`/ab12`) — 리라이트 없음. 같은 SPA가 그대로 응답
   GET /ab12               ─▶ Pages 엣지: 빌드 타임 prerender된 정적 HTML(+OG 메타) 반환 (읽기 비용 0)
...
- **단축 도메인**: `kit.dgedu.link` → Pages 프로젝트에 커스텀 도메인 연결. 앱 경로가 `/<id>`라 **리라이트 불필요**(같은 SPA가 `/<id>` 응답).
...
// data/items.json — 배열 (꾸러미당 여러 행)
{
  ...
  "item_key": "v1",          // 항목 키 — URL /:kitId/:itemId 의 itemId
...
### 5.5 shortId 발급 규칙
> **동일성**: `kits.id` == `shortId` == 라우트 `:kitId` 는 모두 같은 값. **앱 경로 자체가 `/<id>`** 이고 단축 도메인 `kit.dgedu.link/<id>` 도 동일 경로다(`/k/` 접두어·리라이트·리졸버 테이블 모두 없음). 항목은 `/<id>/<itemId>`.
- **알파벳**: 혼동문자 제외 base31 `23456789abcdefghijkmnpqrstuvwxyz`(0/O,1/l/I 제거). 4자 ≈ 92만 조합.
- **발급(자동 — 결정 B)**: 편집자는 시트 `id` 칸을 **비워 둔다**. **발행 버튼(GAS)이 빈 칸에 base31 4자를 생성해 시트에 되쓰고**(영구 고정 → URL 안정) 커밋한다(`gas/publish.gs`의 `ensureKitIds_`). 빌드 ETL은 형식·유일성을 검증하고, 빈 id가 들어오면 안전망으로 임시 생성+경고(`scripts/shortid.ts`).
- 시드 `ab12`/`cd34` 등 기존 id 유지. 충돌 회피 재시도 + 잦으면 5자 확장. 욕설·혼동어 블랙리스트(추가 예정).
- 근거: 교사가 구두/판서 공유 → 혼동문자 제외가 오입력 방지. 생성을 GAS(발행)에 두는 이유 = 빌드에서만 생성하면 매 발행마다 id가 바뀌어 링크가 깨지므로, 한 번 생성해 시트에 고정.

### 5.6 빌드/캐시 전략
| 자원 | 처리 | 캐시 |
|---|---|---|
| 홈 `/` | 빌드 타임 정적 페이지(전체 kits 데이터 인라인/번들) | 엣지 영구 캐시(immutable, 해시 파일명) |
| 뷰어 `/:kitId` | **kit별 빌드 타임 prerender**(OG 메타 포함) | 엣지 영구 |
...
| `/:kitId` | 뷰어(첫 항목 자동) | **kit별 prerender + OG** | 예: `/ab12` |
| `/:kitId/:itemId` | 뷰어(특정 항목) | prerender + OG | itemId = items.item_key, 예: `/ab12/v1` |
| `kit.dgedu.link/<id>` | 단축 도메인 | 동일 SPA | 앱 경로와 **동일 경로**(`/<id>`) — 리라이트 불필요. 진입 후 URL도 `/<id>` 유지 |
| `/api/kits/:id/stats` | 카운트 조회 | Function | no-store |
| `/api/kits/:id/view` `/like` | 카운트 쓰기 | Function | no-store |

활용 안내 모달은 라우트가 아니라 홈 상태(`showGuide`)로 토글. **어드민 라우트 없음**(C4).

### 7.2 컴포넌트 트리
```
<App> (RR7 root, 헤더/토큰/폰트)  — 토큰은 edu-link index.css 복사(app/styles/tokens.css)
├── <HomeRoute>                          # /
│   ├── <Header>                         # 좌:로고+워드마크+kit.dgedu.link / 우:'활용 안내' 버튼만 (교육청 라벨 없음 — README 25행)
...
│   ├── <EmptyState>                     # 결과 없을 때
│   └── <UsageGuide>                     # showGuide 토글, 교사용 사용법 모달(데이터구조 X)
└── <ViewerRoute>                        # /:kitId/:itemId?
...
> 1. **헤더 우측 교육청 라벨 없음**(README 25행): 우측엔 `활용 안내` 버튼만(이전 '데이터 구조' 버튼은 제거 — 스키마는 docs로 이동). 교육청 정체성은 좌측 워드마크/`kit.dgedu.link`로 충분.
...
| 활용 안내 모달 | 자체 Modal(포커스 트랩·Escape·aria-modal·반환 포커스) — HeroUI Modal로 교체 가능 |
...
- 뷰어 `selItem`: URL 경로(`/:kitId/:itemId`)가 SoT.
- `sidebarOpen`/`copied`/`showGuide`: 로컬 컴포넌트 상태.
...
> **구현·검증 완료(P3)**: `functions/api/kits/[id]/{stats,view,like}.ts` + `functions/_shared.ts`. 로컬 `wrangler pages dev` + 로컬 D1로 e2e 확인 — 조회 +1·1인1일 dedupe(중복 무증가, 방문자별 분리 증가)·좋아요 멱등 토글(+1/-1, 재요청 무증가)·방문자별 liked. 뷰어 클라(`app/lib/api.ts`, ViewerPage)는 진입 시 view 집계 + stats 수신, 좋아요 낙관적 업데이트(실패 시 롤백·시드 폴백). 배포 절차는 [DEPLOY.md](DEPLOY.md).
...
- 단축 도메인은 앱 경로와 동일(`/<id>`)이라 리라이트조차 없음 — OG는 `/<id>` prerender가 그대로 제공.
...
- ✅ **완료**: view/like/stats Functions + **D1 `edu-link-db`**(`edukit_stats`/`edukit_likes`/`edukit_view_dedupe`, dedupe·멱등) — 로컬 e2e 검증. 뷰어 낙관적 UI + 시드 폴백, 링크 복사. 배포 절차 [DEPLOY.md](DEPLOY.md).
- ⏳ **남음**: kit별 prerender + **동적 OG 이미지**(빌드 생성, 현재 정적 OG 메타), lite-youtube 파사드(현재 선택 항목만 iframe), 이미지 R2 미러/업로드, 홈 카드 라이브 카운트(배치 stats), 본문 클라 DOMPurify, Pages 배포·도메인 연결.
...
## 11. 디렉터리 구조 (현 구현 — 단독 레포, Vite SPA)

```
edu-kit/  (현 eduinside/edu-kit — 단독 레포)
├─ index.html · vite.config.ts        # Vite SPA 엔트리/설정
├─ app/
│  ├─ main.tsx                 # 엔트리(BrowserRouter), tokens.css import
│  ├─ App.tsx                  # 라우트: / , /:kitId , /:kitId/:itemId
│  ├─ routes/
│  │  ├─ HomePage.tsx          # 홈 갤러리(필터·그리드·빈상태·활용안내)
│  │  └─ ViewerPage.tsx        # 뷰어(상단바·사이드바·서브헤더·본문)
│  ├─ components/
│  │  ├─ KitCard.tsx · SegmentedControl.tsx · Modal.tsx · UsageGuide.tsx
│  │  └─ viewer/               # Sidebar.tsx, ContentPane.tsx(intro/video/image/text/empty)
│  ├─ styles/tokens.css        # edu-link index.css 복사 + 단계색·교과팔레트·sk-rich/sk-callout
│  └─ lib/                     # data.ts, design.ts, stats.ts (P1; sanitize.client/youtube는 P3)
├─ public/_redirects           # Pages SPA 폴백(/* → index.html)
├─ functions/                  # (P3) Cloudflare Pages Functions
│  └─ api/kits/[id]/           # stats.ts, view.ts, like.ts
├─ data/
...
│  ├─ field-map.ts             # 시트→정본 컬럼 매핑 · markdown.ts · sanitize.ts · types.ts
│  ├─ shortid.ts               # base31 shortId 생성(안전망; 발급은 GAS)
...
│  └─ publish.gs               # 빈 id 자동부여(되쓰기) + 3탭 직렬화 + Contents API PUT
...
| **D-도메인** | `kit.dgedu.link` 연결 | Pages 커스텀 도메인(앱 경로=`/<id>`, 리라이트 불필요) | DNS·도메인 소유 확인 필요. SPA 폴백은 `public/_redirects` |
